...
class TestTkrzw(unittest.TestCase):
  ...
  def testBasic(self):
    ...
    status, value = dbm.CompareExchangeAndGet("xyz", RemoteDBM.ANY_DATA, RemoteDBM.ANY_DATA)
    self.assertEqual(Status.INFEASIBLE_ERROR, status)
    self.assertEqual(None, value)
    status, value = dbm.CompareExchangeAndGet("xyz", None, "abc")
    self.assertEqual(Status.SUCCESS, status)
    self.assertEqual(None, value)
    status, value = dbm.CompareExchangeAndGet("xyz", RemoteDBM.ANY_DATA, "def")
    self.assertEqual(Status.SUCCESS, status)
    self.assertEqual("abc", value)
    status, value = dbm.CompareExchangeAndGet("xyz", RemoteDBM.ANY_DATA, b"ghi")
    self.assertEqual(Status.SUCCESS, status)
    self.assertEqual(b"def", value)
    status, value = dbm.CompareExchangeAndGet("xyz", "ghi", None)
    self.assertEqual(Status.SUCCESS, status)
